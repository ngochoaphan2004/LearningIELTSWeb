...
const MyCourses = () => {
  ...
  useEffect(() => {
    const fetchMyCourses = async () => {
      try {
        const res = await apiClient.get('/api/users/me/courses');
        setCourses(res.data);
      } catch (error) {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg-primary)' }}>
      ...
      <div className="container" style={{ padding: '4rem 20px 2rem', flex: 1 }}>
        <div style={{ marginBottom: '3rem', textAlign: 'center' }}>
          <h1 style={{ fontSize: '2.5rem', marginBottom: '0.5rem' }}>Khóa Học Của Tôi</h1>
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>Tiếp tục hành trình học tập của bạn.</p>
        </div>

        {courses.length === 0 ? (
          <div style={{ textAlign: 'center', marginTop: '4rem' }}>
            <BookOpen size={64} color="var(--text-secondary)" style={{ margin: '0 auto', opacity: 0.5, marginBottom: '1rem' }} />
            <h3 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>Bạn chưa đăng ký khóa học nào!</h3>
